...
const fallbackPaymentMethods: PaymentMethod[] = [
  { id: "cod", name: "Cash on Delivery (COD)", slug: "cod", is_active: true, sort_order: 10 },
  { id: "bank_pay", name: "Bank Pay after order review", slug: "bank_pay", is_active: true, sort_order: 20 },
];
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        setPaymentMethod((current) => methods.some((method) => method.slug === current) ? current : methods[0].slug);
      });
  }, []);
      ...
      <div className="flex min-h-screen flex-col bg-background text-foreground">
        ...
        <main className="flex flex-1 items-center justify-center px-6 pb-16 pt-28">
          <div className="w-full max-w-lg text-center">
            ...
            <p className="mt-6 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Order review pending</p>
            <h1 className="mt-2 text-3xl font-bold">Order request received</h1>
            <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">
              Your order has been saved. Delivery, stock and payment details will be reviewed before fulfilment continues.
            </p>
            <div className="mx-auto mt-7 max-w-xs rounded-lg border border-border px-5 py-4">
              <span className="block text-xs font-medium uppercase tracking-[0.12em] text-muted-foreground">Order reference</span>
      ...
      <div className="flex min-h-screen flex-col bg-background text-foreground">
        ...
        <main className="flex flex-1 items-center justify-center px-6 pb-16 pt-28 text-center">
          <div>
            ...
            <Link href="/shop" className="mt-6 inline-flex rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground">Browse jerseys</Link>
          </div>
        </main>
        <Footer />
      </div>
      ...
      <main className="flex-1 px-5 pb-14 pt-28 sm:px-6">
        <form onSubmit={handleSubmit} className="mx-auto max-w-6xl">
          <div className="mb-8 border-b border-border pb-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Delivery details, payment preference and final order review.</p>
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
            <div className="space-y-10">
              <section>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground sm:col-span-2">
              ...
              <section className="border-t border-border pt-8">
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <h2 className="text-lg font-bold">Payment preference</h2>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                  {paymentMethods.map((method) => (
                    <button
                      ...
                      className={`flex min-h-14 items-center gap-3 rounded-xl border px-4 text-left transition-colors ${paymentMethod === method.slug ? "border-primary bg-muted" : "border-border hover:border-primary/40"}`}
                    >
                      <span className="flex size-8 items-center justify-center rounded-full bg-background text-primary"><ClipboardCheck size={15} /></span>
                      <span className="text-sm font-semibold">{method.name}</span>
                      {paymentMethod === method.slug && <Check size={15} className="ml-auto" />}
                    </button>
                  ))}
                </div>

                <div className="mt-4 rounded-xl border border-border bg-muted/50 p-4">
                  <h3 className="text-sm font-semibold">No payment is collected on this page.</h3>
                  <p className="mt-1 text-sm leading-6 text-muted-foreground">TISA reviews stock, delivery and your selected method first. Use only verified instructions associated with your order reference.</p>
                </div>
            ...
            <aside className="border-t border-border pt-6 lg:sticky lg:top-28 lg:rounded-xl lg:border lg:p-6">
                  ...
                  return (
                    <div key={item.id} className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="price-display mt-2 text-base">{formatPriceAED((item.unitPrice + (item.customizationFee ?? 0) + (item.armBadgeFee ?? 0)) * item.quantity)}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              <dl className="mt-5 space-y-3 border-t border-border pt-5 text-sm">
                ...
                <div className="flex justify-between text-muted-foreground"><dt>Delivery</dt><dd>Confirmed after review</dd></div>
              </dl>
              ...
              <button
                ...
                className="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-primary px-5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-35"
              >
                {submitting ? "Submitting..." : "Place order request"}
                <ShieldCheck size={15} />
              </button>
              {submitError && <p className="mt-3 text-center text-xs text-destructive">{submitError}</p>}
              <p className="mt-3 text-center text-xs leading-5 text-muted-foreground">Delivery and payment details are confirmed during review.</p>
            </aside>
          </div>
        </form>
      </main>
