create function public.commit_inventory_import(p_id uuid) returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
  ...
  -- Stable row-lock ordering avoids deadlocks between overlapping admin batches.
  ...
  for item in select value from jsonb_array_elements(batch.payload->'rows') loop
    ...
    if item->>'variantId' is not null then
      ...
      if item->>'productId' is not null then
        ...
        -- Serialize new identity creation, preserve existing catalog data on collision.
    ...
    if nullif(item->>'name','') is not null then
      insert into public.inventory_aliases(source_name,variant_id) values(lower(btrim(item->>'name')),vid) on conflict(source_name) do nothing;
      if exists(select 1 from public.inventory_aliases where source_name=lower(btrim(item->>'name')) and variant_id<>vid) then raise exception 'Source alias collision';end if;
    end if;
...
create function public.reverse_inventory_import(p_id uuid) returns jsonb language plpgsql security definer set search_path='' as $$
declare batch public.inventory_imports%rowtype; item jsonb;r public.inventory%rowtype;
begin
  if not private.is_admin() then raise exception 'Admin access required' using errcode='42501';end if;
  select * into batch from public.inventory_imports where id=p_id for update;
  if not found or batch.status<>'committed' then raise exception 'Only committed imports can be reversed';end if;
  perform set_config('app.stock_import',p_id::text,true);
  perform set_config('app.stock_reason','Reverse Stock Balance import',true);
  for item in select value from jsonb_array_elements(batch.result->'changes') order by value->>'inventoryId' loop
    select * into r from public.inventory where id=(item->>'inventoryId')::uuid for update;
    if not found or r.version<>(item->>'version')::bigint then raise exception 'Stock changed after import. Review a new adjustment instead.' using errcode='40001';end if;
    if (item->>'before')::integer<r.reserved then raise exception 'Original quantity is below current reserved stock';end if;
    update public.inventory set quantity=(item->>'before')::integer where id=r.id;
  end loop;
  update public.inventory_imports set status='reversed' where id=p_id;
  return jsonb_build_object('status','reversed');
end $$;
revoke all on function public.reverse_inventory_import(uuid) from public;
grant execute on function public.reverse_inventory_import(uuid) to authenticated;
